...
public record ESBossLootSubProvider(HolderLookup.Provider registries) implements LootTableSubProvider {
	...
	public void generate(BiConsumer<ResourceKey<LootTable>, LootTable.Builder> consumer) {
		...
		consumer.accept(ESLootTables.BOSS_THE_GATEKEEPER,
			LootTable.lootTable()
				.withPool(LootPool.lootPool()
					.add(LootItem.lootTableItem(ESItems.BOOK.get()).when(BossChallengeCountCondition.max(ESEntities.THE_GATEKEEPER.getId(), 1))))
				.withPool(LootPool.lootPool()
					.add(LootItem.lootTableItem(ESItems.ORB_OF_PROPHECY.get()).when(BossChallengeCountCondition.max(ESEntities.THE_GATEKEEPER.getId(), 1))))
				.withPool(LootPool.lootPool()
