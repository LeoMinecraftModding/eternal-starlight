...
public abstract class BookDefinitionProvider implements DataProvider {
	...
	private static final String SECTIONS_DIRECTORY = DIRECTORY + "/sections";
	protected final ResourceType resourceType;
	protected final PackOutput.PathProvider pathProvider;
	protected final PackOutput.PathProvider sectionPathProvider;
	protected final ExistingFileHelper existingFileHelper;
	protected final CompletableFuture<HolderLookup.Provider> lookupProvider;
	protected final String modid;
	protected final Map<ResourceLocation, BookDefinition> definitions = Maps.newHashMap();
	protected final Map<ResourceLocation, List<ConfiguredBookComponent<?, ?>>> sections = Maps.newHashMap();
	...
	public BookDefinitionProvider(PackOutput output, CompletableFuture<HolderLookup.Provider> lookupProvider, String modId, ExistingFileHelper existingFileHelper) {
		...
		this.sectionPathProvider = output.createPathProvider(PackOutput.Target.RESOURCE_PACK, SECTIONS_DIRECTORY);
	...
	public CompletableFuture<?> run(CachedOutput output) {
		...
		return lookupProvider.thenCompose(provider -> {
			...
			this.sections.forEach((id, components) -> {
				Path path = this.sectionPathProvider.json(id);
				futuresBuilder.add(CompletableFuture.supplyAsync(() -> ConfiguredBookComponent.CODEC.listOf().encodeStart(dynamicOps, components).getOrThrow(msg -> new RuntimeException("Failed to encode %s: %s".formatted(path, msg)))).thenComposeAsync(encoded -> DataProvider.saveStable(output, encoded, path)));
			});
	...
	public void addSection(ResourceLocation id, List<ConfiguredBookComponent<?, ?>> components) {
		this.sections.put(id, components);
	}
