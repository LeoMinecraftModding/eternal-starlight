...
public class BoarwarfHairLayer<T extends Boarwarf> extends RenderLayer<T, BoarwarfModel<T>> {
    private final BoarwarfModel<T> model;

    public BoarwarfHairLayer(RenderLayerParent<T, BoarwarfModel<T>> parent, EntityModelSet modelSet) {
        super(parent);
    ...
    public void render(PoseStack matrixStackIn, MultiBufferSource bufferIn, int packedLightIn, T entityIn, float limbSwing, float limbSwingAmount, float partialTicks, float ageInTicks, float netHeadYaw, float headPitch) {
        if (entityIn.getHairVariant() < 0 || entityIn.getHairVariant() >= BoarwarfVariants.BoarwarfHairVariants.VARIANT_NUM) {
            return;
        }
        ResourceLocation TEXTURE = new ResourceLocation(EternalStarlight.MOD_ID, "textures/entity/boarwarf/hair/" + entityIn.getHairVariant() + ".png");
        if (entityIn.getHairVariant() >= 0 && !entityIn.isInvisible()) {
