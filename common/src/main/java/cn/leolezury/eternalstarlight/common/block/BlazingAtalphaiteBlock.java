...
public class BlazingAtalphaiteBlock extends Block {
	...
	public @Nullable BlockState getStateForPlacement(BlockPlaceContext context) {
		...
		return state == null ? null : (isAffectedByFluid(state, context.getLevel(), context.getClickedPos(), FluidTags.WATER) ? ESBlocks.ATALPHAITE_BLOCK.get().defaultBlockState() : state);
	}

	@Override
	protected BlockState updateShape(BlockState state, Direction direction, BlockState neighborState, LevelAccessor level, BlockPos pos, BlockPos neighborPos) {
		BlockState newState = super.updateShape(state, direction, neighborState, level, pos, neighborPos);
		return isAffectedByFluid(newState, level, pos, FluidTags.WATER) ? ESBlocks.ATALPHAITE_BLOCK.get().defaultBlockState() : newState;
	}
