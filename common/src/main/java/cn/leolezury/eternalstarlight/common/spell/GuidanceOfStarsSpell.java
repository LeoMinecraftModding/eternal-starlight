...
public class GuidanceOfStarsSpell extends AbstractSpell {
	...
	public void onSpellTick(LivingEntity entity, int ticks) {
		if (entity.level() instanceof ServerLevel serverLevel) {
			for (int i = 0; i < 4; i++) {
				serverLevel.sendParticles(ESGlowParticleOptions.getSeek(entity.getRandom(), false, false), entity.getX() + (entity.getRandom().nextDouble() - 0.5) * entity.getBbWidth() * 2, entity.getY() + entity.getBbHeight() / 2d + (entity.getRandom().nextDouble() - 0.5) * entity.getBbHeight() * 2, entity.getZ() + (entity.getRandom().nextDouble() - 0.5) * entity.getBbWidth() * 2, 3, 0.2, 0.2, 0.2, 0);
			}
		}
	}
		...
		if (ticks >= spellProperties().spellTicks() && entity.level() instanceof ServerLevel serverLevel) {
			...
			if (blockPos != null) {
				...
				for (int i = 0; i < 18; i++) {
					...
					ESPlatform.INSTANCE.sendToAllClients(serverLevel, new ParticlePacket(ESGlowParticleOptions.getSeek(entity.getRandom(), true, true), startPos.x + (entity.getRandom().nextFloat() - 0.5) * 0.8, startPos.y + (entity.getRandom().nextFloat() - 0.5) * 0.8, startPos.z + (entity.getRandom().nextFloat() - 0.5) * 0.8, delta.x, delta.y, delta.z));
				}
