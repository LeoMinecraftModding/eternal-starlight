...
public class ESBiomeData {
	...
	public static void bootstrap(BootstrapContext<BiomeData> context) {
		...
		context.register(STARLIGHT_PERMAFROST_FOREST, new BiomeData.Builder(biomeHolderGetter.getOrThrow(ESBiomes.STARLIGHT_PERMAFROST_FOREST), 110, 40).withTemperatures(BiomeData.Temperature.COLD, BiomeData.Temperature.COLD_EXTREME).hasBeaches(false).hasRivers(false).build());
		...
		context.register(SPIRAL_KELP_FOREST, new BiomeData.Builder(biomeHolderGetter.getOrThrow(ESBiomes.SPIRAL_KELP_FOREST), 15, 10).withTemperatures(BiomeData.Temperature.COLD, BiomeData.Temperature.COLD_EXTREME).isOcean(true).build());
